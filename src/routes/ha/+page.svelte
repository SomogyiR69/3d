<script lang="ts">
	...
	import { animate, inView, scroll, timeline, stagger } from 'motion';
	...
	let gridSt = [
		{
			...
			desc: 'Ac tincidunt sapien vehicula erat auctor pellentesque rhoncus. Et magna sit morbi vitae lobortis.'
		},
		{
			icon: `<svg
													class="h-6 w-6 text-white"
			...
			desc: 'Qui aut temporibus nesciunt vitae dicta repellat sit dolores pariatur. Temporibus qui illum aut.'
		},
		{
			icon: `<svg
													class="h-6 w-6 text-white"
			...
			title: 'Simple Queues',
			desc: 'Rerum quas incidunt deleniti quaerat suscipit mollitia. Amet repellendus ut odit dolores qui.'
		},
		{
			icon: `<svg
													class="h-6 w-6 text-white"
			...
			desc: 'Ullam laboriosam est voluptatem maxime ut mollitia commodi. Et dignissimos suscipit perspiciatis.'
		},
		{
			icon: `<svg
													class="h-6 w-6 text-white"
			...
			title: 'Powerful API',
			desc: 'Ab a facere voluptatem in quia corrupti veritatis aliquam. Veritatis labore quaerat ipsum quaerat id.'
		},
		{
			icon: `<svg
													class="h-6 w-6 text-white"
			...
			desc: 'Quia qui et est officia cupiditate qui consectetur. Ratione similique et impedit ea ipsum et.'
		}
	];

	const gridStuff = [...gridSt];

	let heroImg: HTMLImageElement;
	let heroTitle1: HTMLSpanElement;
	let heroTitle2: HTMLSpanElement;
	let heroCTA: HTMLAnchorElement;
	let heroText: HTMLParagraphElement;
	let heroForm: HTMLDivElement;
	let featureImg: HTMLImageElement;
	let staggeredGridStuff: HTMLDivElement[] = [];
	let loaderBG: HTMLDivElement;
	let loaderText: HTMLSpanElement;
	let testImg: HTMLDivElement;
	let blog: HTMLDivElement;
	let imgColoration: HTMLDivElement;
	...
	onMount(() => {
		...
		const splitHero1 = new SplitType(heroTitle1, {
			types: 'chars'
		});
		const splitHero2 = new SplitType(heroTitle2, {
			types: 'chars'
		});
		...
		const animateHeroForm = [
			heroForm,
			{ opacity: [0, 1] },
			{ duration: 1.2, easing: [0.22, 0, 0.36, 1], at: '-0.5' }
		];
		const animateSplitHeroText = [
			splitHeroText.words,
			{ y: ['100%', '0'] },
			{ delay: stagger(0.05), duration: 0.4, easing: [0.22, 0, 0.36, 1], at: '<' }
		];

		const animateHeroCTA = [
			heroCTA,
			{ width: ['0%', '56%'] },
			{ duration: 1.2, easing: [0.22, 0, 0.36, 1], at: '<' }
		];

		const animateSplitHero1 = [
			splitHero1.chars,
			{ opacity: [0, 1] },
			{ delay: stagger(0.05), duration: 0.7, at: '<' }
		];
		const animateSplitHero2 = [
			splitHero2.chars,
			{ opacity: [0, 1] },
			{ delay: stagger(0.05), duration: 0.7, at: '-0.3' }
		];

		const animateHeroImg = [
			heroImg,
			{ x: ['100%', '0'] },
			{ duration: 1.2, easing: [0.22, 0, 0.36, 1], at: '-0.6' }
		];
		...
		timeline([
			...
			animateSplitHero1,
			animateSplitHero2,
			animateSplitHeroText,
			animateHeroForm
		]);

		/* inView(featureImg, ({ target }) => {
			animate(target, { scale: [0.5, 1] }, { duration: 1.2, easing: [0.22, 0, 0.36, 1] });
		}); */

		scroll(animate(featureImg, { scale: [0.5, 1] }), {
			target: featureImg,
			offset: ['0 1', '0.3 0.7']
		});

		staggeredGridStuff.forEach((item, itemIndex) => {
		...
		inView(blog, (info) => {
			...
			animate(info.target, { scale: [0.5, 1] }, { duration: 0.9, easing: [0.22, 0, 0.36, 1] });
		});

		scroll(
			animate(imgColoration, {
				filter: ['hue-rotate(0deg)', 'hue-rotate(30deg)']
			}),
			{ target: imgColoration, offset: ['1 1', '1 0.5'] }
		);
	});
</script>
				...
				<div class="overflow-hidden rounded-lg bg-white shadow-md ring-1 ring-black ring-opacity-5">
					...
					<div class="pb-6 pt-5">
						...
						<div class="mt-6 px-5">
							<a
								href="#"
								class="block w-full rounded-md bg-gradient-to-r from-teal-500 to-cyan-600 px-4 py-3 text-center font-medium text-white shadow hover:from-teal-600 hover:to-cyan-700"
								>Start free trial</a
							>
						</div>
						<div class="mt-6 px-5">
		...
		<main>
						...
						>
							<div class="lg:py-24">
								<a
									bind:this={heroCTA}
									href="#"
									class="inline-flex items-center overflow-hidden rounded-full bg-black p-1 pr-2 text-white hover:text-gray-200 sm:text-base lg:text-sm xl:text-base"
								>
									<span
										class="overflow-hidden whitespace-nowrap rounded-full bg-gradient-to-r from-teal-500 to-cyan-600 px-3 py-0.5 text-sm font-semibold leading-5 text-white"
										>We're hiring</span
									>
									<span class="ml-4 overflow-hidden whitespace-nowrap text-sm"
								...
								>
									<span bind:this={heroTitle1} class="block">A better way to</span>
									<span bind:this={heroTitle2} class="block pb-3 sm:pb-5">ship web apps</span>
								</h1>
								<p
									bind:this={heroText}
								...
								<div bind:this={heroForm} class="mt-10 sm:mt-12">
									<form action="#" class="sm:mx-auto sm:max-w-xl lg:mx-0">
										<div class="sm:flex">
											...
											<div class="mt-3 sm:ml-3 sm:mt-0">
												<button
													type="submit"
													class="block w-full rounded-md bg-gradient-to-r from-teal-500 to-cyan-600 px-4 py-3 font-medium text-white shadow hover:from-teal-600 hover:to-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:ring-offset-2 focus:ring-offset-gray-900"
										...
										<p class="mt-3 text-sm text-gray-300 sm:mt-4">
											...
											email, you agree to our <a href="#" class="font-medium text-white"
												>terms of service</a
											>.
										</p>
									</form>
				...
				<div class="mx-auto max-w-md px-6 text-center sm:max-w-3xl lg:max-w-7xl lg:px-8">
					...
					<div class="mt-12">
						<div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
							{#each gridStuff as grid, i}
								<div bind:this={staggeredGridStuff[i]} class="pt-6">
									<div class="flow-root rounded-lg bg-gray-50 px-6 pb-8">
										<div class="-mt-6">
											...
											<p class="mt-5 text-base text-gray-500">{grid.desc}</p>
										</div>
									</div>
								</div>
							{/each}
			...
			<div class="relative bg-gray-900">
				<div class="relative h-56 bg-indigo-600 sm:h-72 md:absolute md:left-0 md:h-full md:w-1/2">
					...
					/>
					<div
						bind:this={imgColoration}
						aria-hidden="true"
						class="absolute inset-0 bg-gradient-to-r from-teal-500 to-cyan-600 mix-blend-multiply"
					></div>
				</div>
				<div
					class="relative mx-auto max-w-md px-6 py-12 sm:max-w-7xl sm:py-20 md:py-28 lg:px-8 lg:py-32"
				>
